...
from paku.features.scan import _read_startup_folder, _read_run_key, _get_desktop_autostart_entries

console = Console(legacy_windows=False)


def _get_cron_reboot_entries() -> List[Tuple[str, str]]:
    """Get crontab @reboot entries for the current user."""
    entries: List[Tuple[str, str]] = []  # (name, program)
    if sys.platform != "linux":
        return entries

    try:
        cmd = ["crontab", "-l"]
        out = subprocess.check_output(cmd, text=True, errors="replace", stderr=subprocess.DEVNULL, timeout=10)
        for line in out.splitlines():
            line_str = line.strip()
            if not line_str or line_str.startswith("#"):
                continue
            if "@reboot" in line_str:
                parts = line_str.split("@reboot", 1)
                cmd_part = parts[1].strip() if len(parts) > 1 else line_str
                entries.append(("@reboot Cron Job", cmd_part))
...
def _get_systemd_enabled_services(user: bool = False) -> List[Tuple[str, str]]:
    """Get systemd services enabled at boot / user login."""
    services: List[Tuple[str, str]] = []  # (name, detail)
    if sys.platform != "linux":
        return services
    ...
    try:
        ...
        if user:
            cmd.append("--user")
        cmd.extend(["list-unit-files", "--type=service", "--state=enabled"])

        out = subprocess.check_output(cmd, text=True, errors="replace", stderr=subprocess.DEVNULL, timeout=10)
        for line in out.splitlines():
            line_str = line.strip()
            if not line_str or line_str.startswith("UNIT FILE"):
                continue
            parts = line_str.split()
            if len(parts) >= 2 and parts[1].lower() == "enabled":
                unit_name = parts[0]
                detail = "User Systemd Service" if user else "Systemd Service"
                services.append((unit_name, detail))
    except (subprocess.TimeoutExpired, subprocess.CalledProcessError, FileNotFoundError, Exception):
        pass
...
def _get_registry_run_entries() -> List[Tuple[str, str, str]]:
    """Get all Run/RunOnce entries (or desktop autostart entries on Linux)."""
    ...
    if sys.platform == "win32":
        ...
        return _get_desktop_autostart_entries()
...
def _get_logon_tasks() -> List[Tuple[str, str]]:
    ...
    if sys.platform == "win32":
        ...
        tasks.extend(_get_cron_reboot_entries())
        tasks.extend(_get_systemd_enabled_services(user=True))
        return tasks
...
def _get_automatic_services() -> Optional[List[Tuple[str, str]]]:
    ...
    if sys.platform == "win32":
        ...
        try:
            return _get_systemd_enabled_services(user=False)
        except Exception as exc:
            print(f"Paku: could not query automatic services via systemctl: {exc}", file=sys.stderr)
            return None
...
def render_autoruns(theme: Theme, mascot_loader: MascotLoader, wait_for_enter: bool = True, animations_enabled: bool = True) -> None:
    ...
    # Registry Run/RunOnce / Desktop Autostart
    reg_text = Text()
    if not registry_entries:
        reg_text.append("  No registry Run/RunOnce entries found.\n" if sys.platform == "win32" else "  No desktop autostart entries found.\n", style=theme.muted)
    else:
    ...
    reg_title = "1. Registry Run/RunOnce" if sys.platform == "win32" else "1. Desktop Autostart Files"
    console.print(Align.center(framed_section(reg_text, reg_title, theme)))
    console.print()
    ...
    # Scheduled Tasks / User Services (Logon)
    task_text = Text()
    if not logon_tasks:
        task_text.append("  No logon-triggered scheduled tasks found.\n" if sys.platform == "win32" else "  No cron @reboot jobs or user systemd services found.\n", style=theme.muted)
    else:
    ...
    task_title = "3. Scheduled Tasks (Logon Trigger)" if sys.platform == "win32" else "3. Cron & User Services (@reboot / logon)"
    console.print(Align.center(framed_section(task_text, task_title, theme)))
    console.print()
    ...
    svc_title = "4. Automatic Startup Services" if sys.platform == "win32" else "4. Enabled Systemd Services (Boot)"
    console.print(Align.center(framed_section(svc_text, svc_title, theme)))
    console.print()

    # Disclaimer
    disclaimer_msg = (
        "To disable an entry, use Task Scheduler, Services, or msconfig — Paku doesn't modify these yet."
        if sys.platform == "win32" else
        "To disable an entry, use systemctl, crontab, or autostart desktop files — Paku doesn't modify these yet."
    )
    console.print(Align.center(
        styled_line((disclaimer_msg, theme.muted))
    ))
    console.print()
